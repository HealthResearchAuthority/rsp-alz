...
// Azure auto-generates system topic names, so we need to find the existing one for this storage account
resource existingEventGridSystemTopic 'Microsoft.EventGrid/systemTopics@2022-06-15' existing = if (!createOrUpdate) {
  // The actual name may be auto-generated by Azure Defender, so this might fail
  // Better approach is to always create with createOrUpdate: true and handle duplicates gracefully
  name: systemTopicName
}
